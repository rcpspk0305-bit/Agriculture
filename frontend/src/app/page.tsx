...
import { Upload, Leaf, Cpu, Activity, Camera, Brain, Wind, Thermometer, Droplets, FlaskConical, CloudRain } from "lucide-react";
import axios from "axios";

// Dynamically import Plotly for client-side rendering
const Plot = dynamic(() => import("react-plotly.js").then((mod) => mod.default) as any, { 
  ssr: false, 
  loading: () => (
...
export default function SmartFarmingDashboard() {
  ...
  const [visionResult, setVisionResult] = useState<{ disease: string; confidence: number } | null>(null);
  const [actionPlan, setActionPlan] = useState<string>("");
  const [loadingAI, setLoadingAI] = useState(false);
  const [isUploading, setIsUploading] = useState(false);

  // 1. IOT REAL-TIME POLLING
  useEffect(() => {
    const fetchIot = async () => {
      try {
        const res = await axios.get("http://localhost:8000/iot/live");
        if (res.data.data) {
          const newTemp = res.data.data.temperature;
          const newHum = res.data.data.humidity;
          ...
          setHistory((prev) => {
            const time = [...prev.time, now].slice(-15);
            const temp = [...prev.temp, newTemp].slice(-15);
            const hum = [...prev.hum, newHum].slice(-15);
    ...
    const interval = setInterval(fetchIot, 2000); 
    return () => clearInterval(interval);
  }, []);

  // 2. ML CROP PREDICTION (Auto-triggers)
  useEffect(() => {
    const getCrop = async () => {
      try {
        const res = await axios.post("http://localhost:8000/predict", iotData);
        setCropPrediction(res.data.recommended_crop);
      } catch (e) {}
    };
    if (iotData.N !== undefined) getCrop();
  }, [iotData.N]);

  // 3. COMPUTER VISION HANDLER
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await axios.post("http://localhost:8000/vision/disease-detect", formData, {
  ...
  const generateActionPlan = async (diseaseState: string) => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:8000/chat/action-plan", payload);
  ...
  const glassCardBase = "bg-white/80 backdrop-blur-xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl p-6 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300";
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans px-4 py-6 md:px-8 overflow-hidden selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <header className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-800">
              Project <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500">PS10</span>
            </h1>
        ...
        <div className="flex items-center gap-2 bg-white/80 backdrop-blur-md px-5 py-2.5 rounded-full border border-white shadow-sm">
          ...
          <span className="text-sm font-semibold text-slate-600">Sensors: Live</span>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-8 h-full">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10">
                {loadingAI ? (
                  <div className="flex flex-col space-y-4 animate-pulse pt-2">
                    ...
                    <p className="text-xs text-slate-400 font-medium mt-4">Synthesizing survival parameters...</p>
                  </div>
                ) : actionPlan ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-slate-400 space-y-4 pt-10">
                    ...
                    <p className="text-sm text-center px-4 font-medium">Upload a leaf scan to generate a precise AI action plan.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className={`${glassCardBase} relative flex flex-col items-center justify-center border-dashed border-2 border-slate-200 hover:border-emerald-300 hover:bg-emerald-50/30 group cursor-pointer h-64`}>
              ...
              <div className={`transition-all duration-300 flex flex-col items-center ${visionResult ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                ...
                <p className="text-xs text-slate-500 text-center px-4 font-medium">Drop image here to classify crop diseases instantly.</p>
              </div>
              
              {/* Vision Scan Result Overlay */}
              <div className={`absolute inset-0 bg-white/95 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center z-20 border ${visionResult?.disease === 'Healthy' ? 'border-emerald-200' : 'border-red-200'} shadow-xl transition-all duration-500 ${visionResult ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {visionResult && (
                  <div className="text-center animate-in zoom-in-95 duration-300">
                    ...
                    <p className="text-xs font-bold text-slate-400 tracking-wider uppercase mb-1">Detection Result</p>
                    <h4 className="text-2xl font-black text-slate-800">{visionResult.disease}</h4>
                    <p className="text-sm text-slate-500 mt-1 font-medium">Confidence: {(visionResult.confidence * 100).toFixed(1)}%</p>
                    <button 
                      onClick={(e) => { e.stopPropagation(); setVisionResult(null); setActionPlan(""); }} 
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {/* AI Recommended Crop - Glowing Hero Card */}
        ...
        {/* FULL WIDTH BOTTOM: Plotly Chart */}
        <div className={`${glassCardBase} w-full`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-bold px-3 py-1 bg-slate-100 text-slate-500 rounded-full uppercase tracking-wider">Live Sync</span>
          </div>
          <div className="w-full h-[350px] md:h-[400px] rounded-2xl overflow-hidden bg-white/50 border border-slate-100 relative">
            <Plot
              data={[
                { 
                  x: history.time, y: history.temp, type: 'scatter', mode: 'lines', 
                  name: 'Temperature (°C)', line: { color: '#f97316', width: 4, shape: 'spline' },
                  fill: 'tozeroy', fillcolor: 'rgba(249, 115, 22, 0.05)'
                },
                { 
                  x: history.time, y: history.hum, type: 'scatter', mode: 'lines', 
                  name: 'Moisture (%)', line: { color: '#0ea5e9', width: 4, shape: 'spline' }, yaxis: 'y2',
              ...
              layout={{
                ...
                legend: { orientation: 'h', y: -0.15, font: { family: 'inherit', size: 12, color: '#64748b' } },
                hovermode: 'x unified',
                hoverlabel: { bgcolor: '#ffffff', font: { family: 'inherit' }, bordercolor: '#e2e8f0' }
              }}
